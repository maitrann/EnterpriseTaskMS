...
public sealed class PostgresTaskAccessReader(ApplicationDbContext dbContext)
    : PostgresCommandBase(dbContext), ITaskAccessReader
{
    ...
    public async Task<TaskAccessResult> GetTaskAccessAsync(
        Guid actorUserId,
        Guid taskId,
        ...
        CancellationToken cancellationToken)
    {
        return new TaskAccessResult(
            await TaskExistsAsync(taskId, cancellationToken),
    ...
    public async Task<bool> CanAccessTaskAsync(Guid actorUserId, Guid taskId, CancellationToken cancellationToken)
    {
        const string sql = """
            SELECT EXISTS (
                SELECT 1
    ...
    private async Task<bool> TaskExistsAsync(Guid taskId, CancellationToken cancellationToken)
    {
        const string sql = "SELECT EXISTS (SELECT 1 FROM tasks WHERE id = @taskId);";
        return await ExecuteScalarAsync<bool>(sql, [("@taskId", taskId)], cancellationToken);
    }
}
